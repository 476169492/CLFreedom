...
- (void)scrollLabelIfNeeded
{
    ...
    [UIView animateWithDuration:duration delay:self.pauseInterval options:self.animationOptions | UIViewAnimationOptionAllowUserInteraction animations:^{
        ...
        //if (finished)
        {
            [self performSelector:@selector(scrollLabelIfNeeded) withObject:nil afterDelay:0.25];
        }
    }];
}
...
- (void)refreshLabels
{
    ...
    CGRect labelRect = [self.mainLabel.text boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGRectGetHeight(self.bounds))
...
//                                       constrainedToSize:CGSizeMake(CGFLOAT_MAX, CGRectGetHeight(self.bounds))];
    ...
    if ([Utils isIphone7SystemVersion]) {
