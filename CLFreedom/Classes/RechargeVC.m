...
//在内购项目中创的商品单号
#define MyProduct             @"com.laosiji.freedom01" //1
...
- (void)RequestProductData
{
    ...
    switch (buyType) {
        case MyIAP1:
            product=[[NSArray alloc] initWithObjects:MyProduct,nil];
            break;
        case IAP0p20:
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response{
    ...
    TLLog(@"产品Product ID:%@",response.invalidProductIdentifiers);
    TLLog(@"产品付费数量: %d", (int)[myProduct count]);
    // populate UI
    for(SKProduct *product in myProduct){
        TLLog(@"product info");
        TLLog(@"SKProduct 描述信息%@", [product description]);
        TLLog(@"产品标题 %@" , product.localizedTitle);
        TLLog(@"产品描述信息: %@" , product.localizedDescription);
        TLLog(@"价格: %@" , product.price);
        TLLog(@"Product id: %@" , product.productIdentifier);
    }
    SKPayment *payment = nil;
    
    switch (buyType) {
        case MyIAP1:
            payment  = [SKPayment paymentWithProductIdentifier:MyProduct];    //支付1
        case IAP0p20:
...
{
    TLLog(@"%@",  [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding]);
}
